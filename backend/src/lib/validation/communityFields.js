...
/**
 * Reglas de validación compartidas por communities, members y requests para campos de texto obligatorio.
 */
...
// Valida un formato simplificado de CIF para pruebas: H seguido de 8 cifras.
...
// Valida formato de números de vía (ej: "12", "12B", "12-14", "12/3").
...
const postalCodeSchema = z.string().trim()
  .refine((value) => SPAIN_POSTAL_CODE_REGEX.test(value), 'El código postal debe ser un código postal válido');

const accessCodeSchema = z.string().trim().transform((value) => value.toUpperCase())
  .refine((value) => ACCESS_CODE_REGEX.test(value), 'El código de acceso debe ser un código de comunidad válido');
