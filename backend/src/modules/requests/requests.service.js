// Servicio de requests: resuelve el ciclo completo de las solicitudes de acceso y actualización.
// Flujo cubierto: usuario autenticado -> permisos/contexto -> repositorio y notificaciones por correo.
// Expone casos de uso para alta, revisión, cancelación, archivo y listados propios o administrativos.
...
function buildRequestResponse(message, result, community) {
  return {
    ...
    details: {
      proposedAlias: result.details.proposedAlias,
...
function mapCommunityPendingRequest(request) {
  const requesterName = [request.user?.firstName, request.user?.lastName]
    .filter(Boolean)
    .join(' ')
    .trim();
...
// --- Solicitudes: comparación de cambios ---
function hasUpdateInfoChanges(input, membership) {
  ...
  return comparisons.some(([nextValue, currentValue]) => (
    normalizeComparableValue(nextValue) !== normalizeComparableValue(currentValue)
  ));
}
...
function getRequesterGreeting(request) {
  const fullName = [request.user?.firstName, request.user?.lastName].filter(Boolean).join(' ').trim();
...
function buildRequestResolutionBody(request, approved, resolutionMessage) {
  ...
  if (approved) {
    return [
      ...
      request.type === 'JOIN' ? 'Te damos la bienvenida a la comunidad.' : 'Los datos de tu comunidad han sido actualizados.',
      '',
      ...reasonLine,
...
async function notifyRequestResolution(request, status, resolutionMessage) {
  ...
  const subject = approved ? `SIGECO - Solicitud aprobada en ${communityName}` : `SIGECO - Solicitud rechazada en ${communityName}`;
  const body = buildRequestResolutionBody(request, approved, resolutionMessage);

  try {
    await mailService.sendMail({ to: targetEmail, subject, text: body.join('\n') });
  }
  catch (error) {
    console.warn('No se ha podido enviar el correo de resolución de solicitud', { requestId: request.id, communityId: request.communityId, status, error });
  }
}
...
async function notifyNewRequestToLeaders({ request, details, community, requestComment }, requestsRepository) {
  ...
  try {
    await mailService.sendMail({ to: recipients.join(', '), subject: `SIGECO - Nueva solicitud en ${community.name}`, text });
  }
  catch (error) {
    console.warn('No se ha podido enviar el correo de nueva solicitud', { requestId: request.id, communityId: community.id, error });
  }
}
...
async function requireAdministrativeMembership(userId, communityId) {
  // Requests reutiliza la misma política administrativa que members para no duplicar reglas.
...
async function requireReviewableRequest(userId, requestId, requestsRepository) {
  ...
  // La autorización se calcula contra la comunidad a la que pertenece la propia solicitud.
...
async function createJoinRequest(context, input, requestsRepository) {
  ...
  // El código solo identifica la comunidad destino; el alta real sigue necesitando revisión administrativa.
...
async function createUpdateInfoRequest(context, input, requestsRepository) {
  // UPDATE_INFO exige pertenencia previa y un cambio real respecto a los datos actuales.
  ...
  if (!membership || !hasUpdateInfoChanges(input, membership)) {
...
// Se usa junto al perfil para que el usuario vea el estado de sus solicitudes aún visibles.
...
// --- Solicitudes propias: POST de cancelación/archivo ---
async function cancelRequest(userId, requestId, requestsRepository) {
  ...
  // Cancelar conserva el historial; solo cambia el estado de una solicitud que ya no debe seguir abierta.
  const updatedRequest = await requestsRepository.updateRequestState(requestId, {
    status: 'CANCELLED',
...
async function archiveRequest(userId, requestId, requestsRepository) {
  ...
  // Archivar es solo una decisión de visibilidad para el usuario; no cambia la resolución real.
...
// --- Revisión administrativa: POST de aprobación/rechazo ---
async function approveRequest(userId, requestId, input, requestsRepository) {
  ...
  try {
    updatedRequest = await requestsRepository.approveRequest({
      requestId,
      resolvedByMembershipId: reviewerMembership.id,
...
async function rejectRequest(userId, requestId, input, requestsRepository) {
  ...
  // Rechazar vacía la bandeja administrativa, pero no modifica memberships ni propiedades del solicitante.
  const updatedRequest = await requestsRepository.rejectRequest({
    requestId,
    resolvedByMembershipId: reviewerMembership.id,
    resolutionMessage: input.resolutionMessage
  });
