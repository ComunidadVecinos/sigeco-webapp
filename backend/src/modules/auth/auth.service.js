// Servicio de auth: resuelve credenciales, crea sesiones y prepara el contexto visible del usuario.
// Flujo cubierto: credenciales -> repositorio/contexto -> cookie de sesión -> respuestas autenticadas.
// Expone casos de uso para registro, login, logout y gestión de contraseña.
...
// La contraseña temporal se genera localmente sin caracteres ambiguos y cumpliendo complejidad.
...
// --- Helpers de errores de validación ---
...
// --- Autenticación: POST de registro ---
...
// --- Autenticación: POST de login ---
// El login devuelve contexto de acceso y persistencia de sesión; el controller envía el `sid` firmado en cookie HttpOnly.
async function loginUser(input, context = {}) {
  ...
  // El login no distingue entre usuario inexistente y contraseña incorrecta.
  if (!user) {
...
// --- Contraseña: POST de cambio ---
// El cambio de contraseña invalida el resto de sesiones, pero conserva la sesión actual.
async function changePassword(userId, currentSessionId, currentPassword, newPassword) {
  ...
  if (!currentMatches) {
    throw buildValidationError('currentPassword', 'La contraseña actual no es válida', 'La contraseña actual no es válida', errorCodes.CURRENT_PASSWORD_INVALID);
  }
...
// --- Contraseña: POST de reseteo ---
// El reseteo no revela si el email existe; si existe, reemplaza la contraseña y luego invalida todas las sesiones.
